...
locals {
  default_env_file_path     = "${path.module}/../../../../apps/iam-http-api/.env.${var.app_env}"
  default_api_build_context = "${path.module}/../../../../apps/iam-http-api"
...
resource "docker_image" "iam_http_api" {
...
resource "docker_container" "iam_http_api" {
  name     = var.api_container_name
  image    = docker_image.iam_http_api.image_id
  restart  = "always"
  must_run = true
  env      = local.env_vars
